...
DiffSequence.diffQueryUnorderedChanges = function (oldResults, newResults,  observer, options) {
    ...
    newResults.forEach(function (newDoc, id) {
        let oldDoc = oldResults.get(id);
        if (oldDoc) {
            if (observer.changed && !EJSON.equals(oldDoc, newDoc)) {
                let projectedNew = projectionFn(newDoc);
                let projectedOld = projectionFn(oldDoc);
                let changedFields = DiffSequence.makeChangedFields(projectedNew, projectedOld);
